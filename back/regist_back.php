...
// IDやパスワードが何も入力されていない場合
if(is_null($userName) || (strcmp($userName, '') != 0) || is_null($password1) || (strcmp($password1, '') != 0) || is_null($password2) || (strcmp($password2, '') != 0)){
	// 登録ページに戻る
	header('Location: ../content/regist.php?e=1');
	// ここでスクリプトを終了する
	exit();
}
...
if(strcmp($password1, $password2) != 0){
	// 登録ページに戻る
	header('Location: ../content/regist.php?e=2');
	// ここでスクリプトを終了する
	exit();
}
...
if(count($userInfo) != 0){
	// 登録ページに戻る
	header('Location: ../content/regist.php?e=3');
	// ここでスクリプトを終了する
	exit();
}
